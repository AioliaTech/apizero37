"""
Parser específico para ClickGarage (clickgarage.com.br) - MODIFICADO
Alteração: Extrair modelo completo em vez de apenas a primeira palavra
"""
...
class ClickGarageParser(BaseParser):
    ...
    def parse(self, data: Any, url: str) -> List[Dict]:
        ...
        for v in veiculos:
            ...
            if is_moto:
                ...
                categoria_final = self.definir_categoria_veiculo(modelo_final, self._clean_version(modelo_completo), opcionais_processados)
    ...
    def _extract_marca_modelo(self, marca_campo: str, modelo_completo: str) -> Tuple[str, str]:
        ...
        MODIFICAÇÃO: Extrai as primeiras duas palavras como modelo para melhor mapeamento
        ...
        # MODIFICAÇÃO: Extrai as primeiras duas palavras como modelo base
        if modelo_completo:
            modelo_words = modelo_completo.strip().split()
            # Pega as primeiras 2 palavras, ou todas se menos de 2
            modelo_final = " ".join(modelo_words[:2]) if len(modelo_words) >= 2 else modelo_completo
        else:
            modelo_final = ""
